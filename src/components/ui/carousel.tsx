...
>(({ className, showDots = true, children, ...props }, ref) => {
...
>((props, ref) => {
  ...
  if (numberOfSlides > 1) {
    return (
      <div
        ref={ref}
        className={`flex justify-center items-center py-1 px-1 bg-[#0000004D] dark:bg-[[#0000004D] rounded-full ${props.className}`}
      >
        {Array.from({ length: numberOfSlides }, (_, i) => (
          <Button
            key={i}
            className={`mx-1 h-2.5 w-2.5 rounded-full p-0 transition-colors ${
              i === currentSlide
                ? "bg-red-500 hover:bg-red-600 border-white border"
                : "bg-black/60 hover:bg-black/80 dark:bg-white/60 dark:hover:bg-white/80 border-white border"
            }`}
